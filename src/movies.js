...
const detailsMovie = document.querySelector(".details-section");
const sectionAllMovies = document.querySelector(".allMovies");
const sectionDesc = document.querySelector(".section-descr");
const buttonElements = document.querySelectorAll("button");

ulElement.innerHTML = getPosterImages();
function getPosterImages() {
    const arrImages = moviesData.results.map(movie =>
        `<li class="movie-item"> <img class="movie-image" src="${httpPrefix}${movie.poster_path}"></li>`
    )
    return arrImages.join('');
}
...
function show(index) {
   ...
   buttonElements[index].classList.add(ACTIVE);
    if (index == 0) {
        ...
        movieItems.forEach(section => section.hidden = true);
        movieItems[0].hidden = false;
        hideDetailsBTN();
    } else if (index == 2) {
        movieItems.forEach(section => section.hidden = true);
        movieItems[movieItems.length - 1].hidden = false;
        hideDetailsBTN();
    }
}
...
function setDetailsMovie(ind) {
    movieItems.forEach(section => section.hidden = true);
    movieItems[ind].hidden = false;
    sectionDesc.innerHTML = `<title id="titleMovie">${moviesData.results[ind].title}</title>
    <p id="descrMovie">${moviesData.results[ind].overview}</p>`;
    sectionAllMovies.style.width = "80vw";
}
function hideDetailsBTN0() {
    //sectionElement.hidden = true;
    movieItems.forEach(item => item.hidden = false);
    sectionDesc.innerHTML = "";
    sectionAllMovies.style.width = "100%";
}
function hideDetailsBTN() {
    sectionDesc.innerHTML = "";
    sectionAllMovies.style.width = "100%";
}
